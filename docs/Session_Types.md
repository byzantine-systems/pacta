...
A `Channel(protocol, msg)` from `pacta/session/core` is a handle on a conversation with one peer. The `protocol` parameter records the *whole remaining conversation* as a type. It has no runtime representation: the compiled record holds a pid, an optional monitor and some metadata, and nothing else. What it buys is that using the channel wrongly is a compile error rather than a bug you find in production.

> **Status.** `pacta/session/core`, `pacta/session/duality` and the `gen_statem` driver `pacta/protocol_machine` are implemented, as is the specification layer under `pacta/protocol`, including the generator. The examples below walk a protocol by hand, which is the shortest way to see what the types do, deriving one from a machine is done further down.

- For a worked, runnable version of everything on this page, see [`examples/atm`](https://github.com/byzantine-systems/pacta/tree/main/examples/atm).
...
The first of two entries about *generated* positions, produced by `pacta/protocol/emit` for protocols that repeat. Generated markers are flat names rather than nested markers, which makes these errors the most readable in this catalogue: they name the position you are at and the one you assumed.
...
And a `Channel` has exactly one peer, so three participants cannot be described at all. `pacta/protocol` answers both by not being a type. A protocol is written as an ordinary Gleam value, checked and projected before compilation rather than during it:
...
`pacta/protocol/emit` turns a projected graph into Gleam source, one module per participant. That is the step that makes the specification layer worth having, because it is what lets you write a looping protocol and then *program* against it with the compiler checking every move.
...
**No interleaving composition in the types.** Two protocols that must advance in step, such as a banking service and the authentication it depends on, cannot be woven together by these markers. Filling a `then` hole only puts one protocol *after* another, and the two constrain each other in both directions. `pacta/protocol/weave` does this at the specification layer instead, which is the next section.
...
`pacta/protocol/weave` implements some ideas from Bocchi, Orchard and Voinea's [A Theory of Composing Protocols (2022)](https://arxiv.org/pdf/2203.02461). The three annotations from `pacta/protocol/spec` are what guide it: 
