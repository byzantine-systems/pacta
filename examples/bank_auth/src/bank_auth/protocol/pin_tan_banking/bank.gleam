////
//// The `Bank` view of the `pin_tan_banking` protocol, written by
//// `pacta/protocol/emit`.
////
...
/// Accept `pin` from `Customer`, carrying `bank_auth.Pin`, then continue at
/// `Decision`.
///
pub type AuthenticatingPin

/// Pick one arm and tell `Customer` which:
///
...
/// Send `statement` to `Customer`, carrying `bank_auth.Statement`, then
/// continue at `Banking`.
///
pub type AtStatement

/// Send `id` to `Customer`, carrying `bank_auth.TransactionId`, then continue
/// at `AtTan`.
///
pub type AtId

/// Accept `tan` from `Customer`, carrying `bank_auth.Tan`, then continue at
/// `Decision2`.
///
pub type AtTan
...
/// Accept `details` from `Customer`, carrying `bank_auth.PaymentDetails`, then
/// continue at `Banking`.
///
pub type AtDetails
...
) -> Channel(core.Recv(bank_auth.Pin, Decision), msg) {
...
) -> Channel(core.Choose(core.Send(Nil, Banking), core.Send(Nil, Ended)), msg) {
  core.unchecked_position(channel)
}
...
) -> Channel(core.Offer(core.Recv(Nil, AtStatement), BankingOtherwise), msg) {
  core.unchecked_position(channel)
}
...
) -> Channel(core.Offer(core.Recv(Nil, AtId), core.Recv(Nil, Ended)), msg) {
  core.unchecked_position(channel)
}
...
) -> Channel(core.Send(bank_auth.Statement, Banking), msg) {
  core.unchecked_position(channel)
}
...
) -> Channel(core.Send(bank_auth.TransactionId, AtTan), msg) {
  core.unchecked_position(channel)
}
...
) -> Channel(core.Recv(bank_auth.Tan, Decision2), msg) {
  core.unchecked_position(channel)
}
...
pub fn decision2(
  ...
  core.Choose(core.Send(Nil, AtDetails), core.Send(Nil, Banking)),
  msg,
) {
  core.unchecked_position(channel)
}
...
) -> Channel(core.Recv(bank_auth.PaymentDetails, Banking), msg) {
  core.unchecked_position(channel)
}
...
/// Take the `ok` arm of `Decision`, which continues at `Banking`.
///
pub fn decision_ok(
  channel: Channel(Decision, msg),
) -> Channel(core.Send(Nil, Banking), msg) {
  channel |> decision |> core.choose_left
}

/// Take the `fail` arm of `Decision`, which continues at `Ended`.
///
pub fn decision_fail(
  channel: Channel(Decision, msg),
) -> Channel(core.Send(Nil, Ended), msg) {
  channel |> decision |> core.choose_right
}

/// Follow the `statement` arm of `Banking`, which continues at `AtStatement`.
///
pub fn banking_statement(
  channel: Channel(Banking, msg),
) -> Channel(core.Recv(Nil, AtStatement), msg) {
  channel |> banking |> core.offered_left
}

/// Follow the `payment` arm of `Banking`, which continues at `AtId`.
///
pub fn banking_payment(
  channel: Channel(Banking, msg),
) -> Channel(core.Recv(Nil, AtId), msg) {
  channel
  |> banking
  |> core.offered_right
  |> banking_otherwise
...
/// Follow the `logout` arm of `Banking`, which continues at `Ended`.
///
...
/// Take the `ok` arm of `Decision2`, which continues at `AtDetails`.
///
pub fn decision2_ok(
  channel: Channel(Decision2, msg),
) -> Channel(core.Send(Nil, AtDetails), msg) {
  channel |> decision2 |> core.choose_left
}

/// Take the `fail` arm of `Decision2`, which continues at `Banking`.
///
pub fn decision2_fail(
  channel: Channel(Decision2, msg),
) -> Channel(core.Send(Nil, Banking), msg) {
  channel |> decision2 |> core.choose_right
}
