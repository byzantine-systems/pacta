...
////
//// `pacta/protocol/graph` turns a specification into one flattened state
...
//// `pacta/session/core` and `pacta/protocol_machine`.
////
...
import gleam/result
import gleam/string
import pacta/protocol/graph.{type Arm, type Graph, type State}
import pacta/protocol/spec.{type Protocol}
    ...
    ..list.map(
      list.flatten([
        wrap(
          ...
          <> "` view of the `"
          <> protocol.name
          <> "` protocol, written by `pacta/protocol/emit`.",
        ),
        [""],
        wrap(
        ...
        wrap(
          ...
          <> " it unfolds it into the `pacta/session/core` shape the protocol"
          <> " says it has, one step deep, and everything after that is an"
          <> " ordinary `core` or `pacta/protocol_machine` call.",
        ),
      ]),
      comment("////"),
    )
