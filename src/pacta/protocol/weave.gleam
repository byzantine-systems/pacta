...
//// [A Theory of Composing Protocols (2023)](https://doi.org/10.22152/programming-journal.org/2023/7/6)
...
import pacta/protocol/spec.{type Protocol, type Spec}
...
///
/// This is the acceptance check for code generators. It never chooses among
/// alternatives, and a truncated search is rejected even if it currently
...
pub fn select_unique(
  ...
  case composition {
    Composition(truncated: True, ..) -> Error(SearchTruncated)
    Composition(candidates: [candidate], truncated: False) -> Ok(candidate)
    Composition(candidates:, truncated: False) ->
      Error(UnexpectedCandidateCount(list.length(candidates)))
  }
}
