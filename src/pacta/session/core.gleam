...
//// to an earlier marker, which is `pacta/protocol/emit`'s job rather than
...
//// `gen_statem`'s data is. See `pacta/protocol_machine`.
////
...
/// module written by `pacta/protocol/emit`. A protocol that repeats cannot be
...
/// the shape off a specification `pacta/protocol/graph` had already checked.
